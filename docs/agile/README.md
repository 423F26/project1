...
Only sprints with a recorded completion appear in the SVG, so unfinished Sprint 2 work is retained and validated without appearing in the chart. The chart labels both axes and its inclusive time period, annotates each completion with its date inside the plot, and lists flat periods below the plot. The generator rejects malformed dates, unknown or duplicate task IDs, dates outside a sprint, completion before addition, and missing product-assigned sprint entries. It uses task count rather than timesheet hours; timesheets are not chart inputs.
