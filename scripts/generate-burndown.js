...
function timeline(sprint) {
  ...
  for (const [day, event] of [...events.entries()].sort(([a], [b]) => a.localeCompare(b))) { const date = parseDate(day, 'event'); points.push({ date, remaining }); if (event.added.length) { remaining += event.added.length; changes.push({ date, type: 'added', tasks: event.added, remaining }); points.push({ date, remaining }); } if (event.completed.length) { remaining -= event.completed.length; changes.push({ date, type: 'completed', tasks: event.completed, remaining }); points.push({ date, remaining }); } }
...
function renderSprint(sprint, index) {
  const top = 25 + index * 405; const left = 180; const right = 870; const chartTop = top + 75; const bottom = top + 285; const span = offset(sprint.start, sprint.end) || 1;
  ...
  const coordinate = ({ date, remaining }) => [left + (offset(sprint.start, date) / span) * (right - left), bottom - (remaining / maximum) * 190]; const actual = points.map((point) => coordinate(point).map((value) => value.toFixed(1)).join(',')).join(' ');
  const markers = changes.map((change) => { const [x, y] = coordinate(change); const label = change.type === 'completed' ? `${change.tasks.map((task) => task.Task).join(', ')} · ${dateText(change.date)}` : `+${change.tasks.map((task) => task.Task).join(',')}`; const labelY = Math.max(chartTop + 14, y - 10); return `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="4" class="${change.type}"/><text x="${x.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="middle" class="change-label">${xml(label)}</text>`; }).join('');
  const flat = flatPeriods(sprint, events);
  ...
  return `<g><text x="30" y="${top + 25}" class="title">${xml(sprint.label)} burndown</text><text x="30" y="${top + 47}">Time period: ${dateText(sprint.start)} to ${dateText(sprint.end)} (inclusive)</text><line x1="${left}" y1="${chartTop}" x2="${left}" y2="${bottom}" class="axis"/><line x1="${left}" y1="${bottom}" x2="${right}" y2="${bottom}" class="axis"/><text x="${left - 14}" y="${chartTop + 4}" text-anchor="end">${maximum}</text><text x="${left - 14}" y="${bottom + 4}" text-anchor="end">0</text><text x="55" y="${chartTop + 105}" text-anchor="middle" transform="rotate(-90 55 ${chartTop + 105})" class="axis-label">Remaining tasks</text><text x="${left}" y="${bottom + 40}">${dateText(sprint.start)}</text><text x="${right}" y="${bottom + 40}" text-anchor="end">${dateText(sprint.end)}</text><text x="${(left + right) / 2}" y="${bottom + 65}" text-anchor="middle" class="axis-label">Date</text><line x1="${left}" y1="${bottom - (initial / maximum) * 190}" x2="${right}" y2="${bottom}" class="ideal"/><polyline points="${actual}" class="actual"/>${markers}<text x="${right - 170}" y="${top + 25}" class="legend">— actual · ┄ ideal</text><text x="30" y="${top + 385}" class="note">${xml(flatNote)}</text></g>`;
}
function renderSvg(sprints) { const visible = sprints.filter((sprint) => sprint.tasks.some((task) => task.completed)); const height = Math.max(140, 25 + visible.length * 405); const body = visible.length ? visible.map(renderSprint).join('') : '<text x="30" y="60">No sprint has recorded completed work.</text>'; return `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" width="920" height="${height}" viewBox="0 0 920 ${height}" role="img" aria-labelledby="title description"><title id="title">Agile sprint burndown chart</title><desc id="description">Remaining task count for sprints with recorded completed work.</desc><style>text{font:12px system-ui,sans-serif;fill:#24313d}.title{font-size:18px;font-weight:700}.axis-label{font-weight:700}.note,.change-label{font-size:11px}.legend{fill:#536471}.axis{stroke:#94a3b8}.ideal{stroke:#94a3b8;stroke-width:2;stroke-dasharray:5 4}.actual{fill:none;stroke:#136f63;stroke-width:3}.added{fill:#a16207}.completed{fill:#136f63}</style><rect width="100%" height="100%" fill="#fff"/>${body}</svg>\n`; }
